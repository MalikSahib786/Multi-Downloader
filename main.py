...
app = FastAPI(title="Social Media API", version="SizeFixed.Pro")
...
def try_social_image_scrape(url):
    return None # (Code same as before, kept short for brevity)
            ...
            for f in formats:
                ...
                if f.get('vcodec') == 'none' and f.get('acodec') != 'none':
                    ...
                    audio_option = {
                        ...
                        "res_val": 0,
                        "url": f['url'],
                        "filesize": size  # <--- SENDING RAW BYTES
                    }
...
def stream_content(target: str = Query(...), title: str = Query("file"), size: int = Query(None), key: str = Query(...)):
    ...
    # *** THE FIX: Tell Browser the File Size ***
    if size and size > 0:
    ...
    return StreamingResponse(
        ...
        headers=headers # Sending size header here
    )
