...
app = FastAPI(title="Social Media API", version="PhotoFix.2.0")
...
# --- FIX 1: TWITTER/X IMAGE SCRAPER ---
def try_social_image_scrape(url):
    print("⚠️ yt-dlp failed. Checking for Images (Twitter/Insta)...")
    try:
        # Twitter requires Desktop UA to show meta tags
        ...
        if og_img and og_img.get('content'):
            title = soup.title.string or "Social Media Image"
            return {
                "status": "success",
                "title": title,
...
def extract_media(request: MediaRequest):
    ...
    try:
        ...
        # 2. If yt-dlp fails, Try Image Scraper (Fixes Twitter Photos)
...
def stream_content(target: str = Query(...), title: str = Query("file"), key: str = Query(...)):
    ...
    ext = "mp4"
    if ".jpg" in target_url or "pbs.twimg.com" in target_url or "yt3.ggpht" in target_url: 
        ext = "jpg"
    elif ".mp3" in target_url or "audio" in title.lower(): 
        ext = "mp3"
    
    # B. CRITICAL: Force ASCII Filename
    # This fixes the "stream.json" error. Browsers crash if headers have Emoji/Korean.
    try:
        ascii_title = title.encode('ascii', 'ignore').decode('ascii')
    except:
        ascii_title = "media_file"
    
    safe_title = "".join([c for c in ascii_title if c.isalnum() or c in " _-"])[:50]
    if not safe_title.strip(): safe_title = "downloaded_media"
    
    filename = f"{safe_title}.{ext}"

    def iterfile():
        # C. Use yt-dlp to stream output (Bypasses Python Requests 403)
        cmd = ["yt-dlp", "--no-part", "--quiet", "--no-warnings", "-o", "-", target_url]
        
        if "googlevideo" in target_url:
            cmd.extend(["--user-agent", MOBILE_UA])
        elif "twimg" in target_url:
