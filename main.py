...
import yt_dlp
import subprocess
import requests # Needed for the fallback scraper
from bs4 import BeautifulSoup # Needed for YouTube Posts
...
app = FastAPI(title="Final Social API", version="Native.Fallback.1.0")
...
# --- FALLBACK SCRAPER (For YouTube Posts / Images) ---
def try_social_image_scrape(url):
    print("⚠️ yt-dlp failed. Attempting Fallback Scraper...")
    try:
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'}
        res = requests.get(url, headers=headers, timeout=10)
        soup = BeautifulSoup(res.text, 'html.parser')
        
        # Check OpenGraph Image (Works for YouTube Posts, Insta Images)
...
def extract_media(request: MediaRequest):
    ...
    try:
        ...
        # 2. If yt-dlp fails, Try Fallback (YouTube Posts)
        fallback_data = try_social_image_scrape(url)
        if fallback_data:
            return fallback_data
            
        raise HTTPException(status_code=400, detail="Could not process link. Is it private or deleted?")
...
def stream_content(target: str = Query(...), title: str = Query("file"), key: str = Query(...)):
    ...
    # Determine Extension
    ext = "mp4"
    # Check keywords to guess file type
    if ".jpg" in target_url or ".png" in target_url or "yt3.ggpht" in target_url: ext = "jpg"
    ...
    def iterfile():
        # yt-dlp can download anything to stdout, even images
        cmd = [
            "yt-dlp", "--no-part", "--quiet", "--no-warnings", "-o", "-", target_url
        ]
